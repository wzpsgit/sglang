...
class FunctionProfiler:
    ...
    def start_profile(self, 
                    tp_rank,
                    profile_funcs: Optional[List[str]],     
        ...
        self.profile_funcs = profile_funcs
        if(len(profile_funcs) == 0):
            return

        self.tp_ranks = tp_ranks if tp_ranks is not None else [1]
        if(tp_rank not in self.tp_ranks):
            return 
        
        self.round_counter += 1
        self.step_counters = {} 
        self.profile_steps = profile_steps if profile_steps is not None else [] 
        ...
        if(output_dir is None):
            ...
            self.trace_dir = os.path.join(self.trace_dir, output_dir)
        ...
        self.with_stack = with_stack if with_stack is not None else True
        self.record_shapes = record_shapes if record_shapes is not None else False
        self.profile_memory = profile_memory if profile_memory is not None else False
        self.skip_first = skip_first if skip_first is not None else 0
        self.wait = wait if wait is not None else 0
        self.active = active if active is not None else 1
        self.repeat = repeat if repeat is not None else 0       

        logger.info(f"MX profiler starts: trace_dir={self.trace_dir},\n \
            tp_ranks={self.tp_ranks}, activities={activities}, \
    ...
    def should_profile(self, func_name):
        return self.profile_funcs is not None and func_name in self.profile_funcs
    ...
    def run(self, tp_rank, name, func, *args):
        ...
        logger.info(f"Profiling function={name}, step={self.step_counters[name]}")     
